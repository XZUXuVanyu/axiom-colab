...
function fail(code, message) {
    throw new MemoryStoreError(code, message);
}
...
export class LocalMemoryStore {
    ...
    constructor(root, options = {}){
        ...
        mkdirSync(this.payloadRoot, {
            recursive: true
        });
        mkdirSync(this.stagingRoot, {
            recursive: true
        });
    ...
    ensureOpen() {
        if (this.closed) fail('STORE_CLOSED', 'memory store is closed');
    }
    now() {
    ...
    requireWorkspace(id) {
        workspaceId(id);
        const row = this.database.prepare('SELECT max_bytes, max_objects FROM workspaces WHERE id = ?').get(id);
        if (!row) fail('WORKSPACE_NOT_FOUND', `workspace ${id} does not exist`);
        return row;
    }
    createWorkspace(id, quota = this.defaultQuota) {
        this.ensureOpen();
        workspaceId(id);
        safeInteger(quota.maxBytes, 'maxBytes');
        safeInteger(quota.maxObjects, 'maxObjects');
        try {
    ...
    putPayload(id, bytes, expiresAt = null) {
        ...
        const existing = this.database.prepare('SELECT hash, size, created_at, expires_at, status FROM payloads WHERE workspace_id = ? AND hash = ?').get(id, hash);
    ...
    inspectPayload(id, hash) {
        this.ensureOpen();
        this.requireWorkspace(id);
        hashHex(hash);
        const row = this.database.prepare('SELECT hash, size, created_at, expires_at, status FROM payloads WHERE workspace_id = ? AND hash = ?').get(id, hash);
    ...
    resources(id) {
      ...
      FROM payloads WHERE workspace_id = ?`).get(now, now, now, id);
        return {
            workspaceId: id,
            ...
            expiredObjects: row.expired_objects,
            corruptObjects: row.corrupt_objects
        };
    }
    purgeExpired(id) {
        this.ensureOpen();
    ...
    recover() {
        for (const entry of readdirSync(this.stagingRoot, {
            withFileTypes: true
        }))if (entry.isFile() && basename(entry.name) === entry.name) rmSync(join(this.stagingRoot, entry.name), {
            force: true
        });
        const rows = this.database.prepare('SELECT workspace_id, hash, size FROM payloads').all();
        const referenced = new Set(rows.map((row)=>hashHex(row.hash)));
        for (const prefix of readdirSync(this.payloadRoot, {
            withFileTypes: true
        })){
            ...
            for (const entry of readdirSync(directory, {
                ...
                if (entry.isFile() && !referenced.has(entry.name)) rmSync(join(directory, entry.name), {
                    force: true
                });
            }
        }
...
//# sourceURL=file:///D:/Dev/axiom-colab/source/ts/local-memory-store.ts