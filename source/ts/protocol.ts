import type { JsonSchema, JsonValue } from './harness-types.js'
import {
...
function expectString(value: unknown, path: string, nonEmpty = true): string {
...
function expectProtocolVersion(value: unknown): typeof PROTOCOL_VERSION {
  if (value !== PROTOCOL_VERSION) {
    throw new ProtocolError(
      'UNSUPPORTED_PROTOCOL_VERSION',
      `expected protocolVersion ${PROTOCOL_VERSION}, received ${JSON.stringify(value)}`,
    )
  }
...
function parseCapabilities(value: unknown): string[] {
  ...
  const capabilities = value.map((item, index) =>
    expectString(item, `$.capabilities[${index}]`))
  const seen = new Set<string>()
  for (const capability of capabilities) {
    if (seen.has(capability)) {
      throw new ProtocolError(
        'INVALID_PROTOCOL',
        `$.capabilities contains duplicate capability ${capability}`,
      )
    }
    seen.add(capability)
  }
  ...
  if (missing.length > 0) {
    throw new ProtocolError(
      ...
      `Bridge is missing required capabilities: ${missing.join(', ')}`,
    )
  }
  return capabilities
}
...
function expectSchema(value: unknown, path: string, objectOnly = false): JsonSchema {
...
function parseDescriptor(value: unknown, index: number): ToolDescriptor {
  const path = `$.tools[${index}]`
  const object = expectRecord(value, path)
  expectExactKeys(object, path, [
    'name', 'description', 'whenToUse', 'parameters', 'output',
    'timeoutMs', 'allowParallel', 'sideEffect',
  ])
  ...
  if (!Number.isInteger(timeoutMs) || (timeoutMs as number) <= 0 || (timeoutMs as number) > 3_600_000) {
  ...
  return {
    ...
    parameters: expectSchema(object.parameters, `${path}.parameters`, true) as Record<string, JsonValue>,
...
function parseErrorPayload(value: unknown): BridgeErrorPayload {
  const object = expectRecord(value, '$.error')
  expectExactKeys(object, '$.error', ['code', 'message', 'details'])
...
export function parseToolCallResponse(text: string, expectedId: string): ToolCallSuccess {
  const object = expectRecord(parseJson(text, 'tool stdout'), '$')
  expectProtocolVersion(object.protocolVersion)
  if (object.ok === true) {
    expectExactKeys(object, '$', ['protocolVersion', 'id', 'ok', 'result'])
  ...
  if (object.ok === false) {
    expectExactKeys(object, '$', ['protocolVersion', 'id', 'ok', 'error'])
    if (object.id !== expectedId) {
      throw new ProtocolError('CALL_ID_MISMATCH', `expected id ${expectedId}, received ${JSON.stringify(object.id)}`)
    }
...
export function makeToolCallRequest(
  id: string,
  tool: string,
  args: unknown,
