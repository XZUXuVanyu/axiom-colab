...
import {
  ...
  type ValidationConfinement,
  type ValidationExecutionBackend,
  type ValidationResourceLimits,
...
export interface WslValidationBackendOptions {
...
export class WslValidationBackend implements ValidationExecutionBackend {
  readonly confinement: ValidationConfinement = Object.freeze({
  ...
  constructor(options: WslValidationBackendOptions) {
  ...
  async run(
    ...
    const args = [
      ...
      '--bind', linuxRoot, '/workspace',
      '--chdir', sandboxCwd(command.cwd),
      '--clearenv',
      '--setenv', 'HOME', '/tmp',
