...
test('candidate changes invalidate promotion eligibility for an earlier record', async () => {
  ...
  assert.equal(runner.isValidationAuthentic(first.snapshot.snapshotHash, first.record), true)
  assert.equal(runner.isValidationAuthentic(changed.snapshot.snapshotHash, first.record), false)
  assert.equal(runner.isValidationAuthentic(first.snapshot.snapshotHash, { ...first.record }), false)
  assert.equal(runner.isPromotionEligible(first.snapshot.snapshotHash, first.record), false)
  assert.equal(first.record.confinement.backend, 'none')
