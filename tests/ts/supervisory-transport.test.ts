...
function snapshot(workspaceId: 'workspace:alpha' | 'workspace:beta') {
  return {
    workspaceId, goalId: null, currentPlan: null, tools: [], candidates: [], timeline: [],
...
test('supervisory transport rejects malformed, oversized, and authority-changing requests', async () => {
  const transport = new SupervisoryTransport({ workspaces: () => [], async inspect() { throw new Error('unused') } } as any, 256)
