...
test('supervisory process boundary serves correlated JSON lines without stdout diagnostics', async () => {
  ...
  const request = [
    JSON.stringify({ protocolVersion: '1.1', id: 'one', operation: 'list-workspaces' }),
    JSON.stringify({ protocolVersion: '1.1', id: 'two', operation: 'inspect', workspaceId: 'workspace:alpha', goalId: null }),
    JSON.stringify({ protocolVersion: '1.1', id: 'three', operation: 'approve' }),
  ].join('\n')
...
test('supervisory process boundary contains oversized lines and resumes at framing boundary', async () => {
  const runner = new ProcessRunner()
  const valid = JSON.stringify({ protocolVersion: '1.1', id: 'after', operation: 'list-workspaces' })
  const result = await runner.run(process.execPath, {
    args: [join(process.cwd(), 'tests/fixtures/supervisory-transport-child.mjs')],
